<script setup lang="ts">
  ...
  interface CarouselProps {
    ...
    stopOnHover?: boolean;
    /** Show the fade effect at both ends */
    overlay?: boolean;
  }
  ...
  const styles = computed<StylesType>(() => ({
    ...
    overlayEffect: overlay
      ? `linear-gradient(${type === 'horisontal' ? 'to right' : 'to bottom'}, transparent, #000, transparent)`
      : 'none',
  }));
  // --------------------------------------------
...
<template>
  <div dir="ltr" :class="['carousel-cont', `animation-${styles.animationName}`]">
    <div>
      <slot></slot>
    </div>
...
<style scoped>
  .carousel-cont {
    ...
    gap: v-bind(gap);
    overflow: hidden;
    mask-image: v-bind('styles.overlayEffect');
  }
